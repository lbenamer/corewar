...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/07/17 18:36:53 by lbenamer          #+#    #+#             */
/*   Updated: 2017/07/17 18:36:55 by lbenamer         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static	void	disp_error(int error, char *arg)
{
	...
	if (error == -5)
		ft_printf("Error: file %s has a code size not valid\n", arg);
	if (error == -6)
...
void			disp_usage(int error, char *arg)
{
	...
	ft_printf("usage : ./corewar [ -d N | -l | -t N | -v ] | [ -n N ]");
	ft_printf("</path/champ.cor> <...>\n");
...
void			disp_player(t_dt *dt)
{
	while (dt)
	{
		ft_printf("Player: [ %d ] - Size : %d oct - Name: %s -> %s \n",
