...
static	void			clock(t_pcs *pcs, t_vm *vm, t_ins *ins)
{
	...
	{
		pcs->pc &= 0x0fff;
		if ((lx = vm->ram[pcs->pc]) > 0 && lx < 17)
		{
			if (get_cycles(lx) == pcs->cycle)
			{
				g_ops.all & V ? reverse(pcs->pc, lx, pcs->color) : 0;
				(g_ops.text & 1) ? ft_printf("P  %5d |", pcs->id) : 0;
				ins[lx](pcs, vm);
				pcs->cycle = 1;
				g_ops.all & V ? blink_pos(pcs->pc, lx, pcs->color) : 0;
			}
			else
				++pcs->cycle;
		}
...
void					run_pcs(t_pcs *pcs, t_vm *vm)
{
	...
	{
		getch() != ERR ? pause_vm() : 0;
		if (g_ops.dump && vm->cycles == g_ops.dump + 1)
			return ;
		(g_ops.text & 2) ? ft_printf("its now cycle : %d\n", vm->cycles) : 0;
		g_ops.all & V ? print_cycles(vm->cycles) : 0;
		clock(pcs, vm, tb_ins);
		pcs = place_max(pcs);
		if (!die && ++n_check)
