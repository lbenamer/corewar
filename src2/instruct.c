...
void ldi(t_pcs *pcs, t_vm *vm)
{
	...
	pc += load_param(pcs, vm->ram, opc | 0x10, 2, &p[1]);
	pc += load_param(pcs, vm->ram, opc | 0x4, 3, &p[2]);
	p[0] += p[1];
	p[0] %= IDX_MOD;
...
void	sti(t_pcs *pcs, t_vm *vm)
{
	...
	buf = (unsigned char *) & p[0];
	buf = mem_rev(buf, 4);
	store(vm->ram, buf, 4, pc + p[1]);
	printf(" - store to index : %d - ", pc + p[1]);
	printf("\n");
}
